...
function SignupModal({ onClose, onOpenLogin, visible }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!RECAPTCHA_SITE_KEY) {
      alert("Missing reCAPTCHA site key. Set VITE_RECAPTCHA_SITE_KEY in Vercel Environment Variables.");
      return;
    }
    ...
    // Save data and trigger the reCAPTCHA challenge on click
    setPendingSignupData(data);
    recaptchaRef.current?.execute();
  };

  const onRecaptchaChange = (token) => {
    if (!token) return; // user closed challenge / failed
    ...
    // Continue your existing flow, but include the token
    (async () => {
      try {
  ...
  const proceedWithRegistration = async (data, recaptchaToken) => {
    ...
    // Attach token for Django verification
  ...
  const getProvinceName = (code) => provinces.find((prov) => prov.prov_code === code)?.name || "";
  const getCityName = (code) => cities.find((city) => city.mun_code === code)?.name || "";
